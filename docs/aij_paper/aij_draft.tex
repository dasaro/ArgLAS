...
$0.67$--$0.72$ on the multi-extension targets and $0.86$ on complete/grounded
--- the near-functional single-extension case where local features suffice ---
against the LAS encodings' verified exact recovery. The interpretable depth-6
...
plateau well below the LAS encodings' verified exact recovery, and closest only
...
\emph{are} the extensions, is needed and, uniquely, is provably exact.
...
learns the acceptability semantics itself as an explicit, reusable, and
\emph{verified} logic program, which is our contribution.
...
level uses \textsc{anthem}~\citep{anthem2020,anthem2023}, now maintained as
\textsc{anthem}~2.0~\citep{fandinno2025anthem2}, with \textsc{Vampire}
\citep{vampire}. That line verifies (and refactors) \emph{hand-written} ASP
programs against specifications; applying it to \emph{learned} programs --- and
using it to falsify and repair a published encoding --- is, to our knowledge,
new, and is what our C2 novelty claim refers to.
...
\bibitem[Fandinno et al.(2025)]{fandinno2025anthem2}
J.~Fandinno, C.~Glinzer, Z.~Hansen, J.~Heuer, Y.~Lierler, V.~Lifschitz,
T.~Schaub, T.~Stolzmann.
Anthem 2.0: automated reasoning for answer set programming.
\emph{Theory and Practice of Logic Programming} (ICLP), 2025.
