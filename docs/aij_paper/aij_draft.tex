...
Figure~\ref{fig:baseline_comparison} reports clean-label recovery. The strongest
feature learner (the random forest --- so this is not a strawman) tops out at MCC
$0.67$--$0.72$ on the multi-extension targets and $0.86$ on
complete/grounded\footnote{Skeptical-complete acceptance coincides with
...
tree is lower ($0.55$--$0.82$), and under noise all degrade (e.g.\ the grounded
tree $0.86\to0.45$ at $q{=}0.2$) where the LAS route is data-recoverable. Two
things separate the baseline from LAS: acceptance is a \emph{global} property of
the extension structure that local features can only approximate, and a
per-argument classifier structurally \emph{cannot represent the set of
extensions} at all. We considered other symbolic ILP systems as baselines
(Popper, Aleph) but they cannot express the negation-as-failure and choice-rule
semantics the task requires, nor learn from noisy examples --- the study's
central axis (Section~\ref{sec:related}).
...
which no bounded-depth tree over local features can express, so the tree tops out
at MCC $0.86$ rather than the LAS program's exact $1.0$. For credulous-stable
...
\emph{are} the extensions, is needed --- and reaches exact recovery
(Section~\ref{sec:exp1}), with the curated encodings proved correct outright
...
learns the acceptability semantics itself as an explicit, reusable logic
program with correctness proofs, which is our contribution.
...
observational predicate learning limits discussed by \citet{law2020}. Other
modern ILP systems --- Popper~\citep{cropper2021popper} and
Aleph~\citep{srinivasan2001aleph} --- are not applicable baselines here: they
learn definite/normal Prolog programs and support neither the negation-as-failure
and choice-rule constructs the acceptability encodings require nor learning from
noisy examples, the study's central axis; our non-LAS comparison
(Section~\ref{sec:eval}) therefore uses feature-based classifiers instead.
...
Experiment~2 re-analyzes. A parallel strand \emph{posits} new semantics designed
to fit human judgement --- SCF2~\citep{cramer2019scf2} and the cognitively
motivated proposals discussed by \citet{cramer2023cognitive} --- selecting or
hand-crafting a semantics that best matches behaviour. Our approach is
...
Technical report, arXiv:2404.18672, 2024.
...
acceptability.
Technical report, arXiv:1908.08406, 2019.
