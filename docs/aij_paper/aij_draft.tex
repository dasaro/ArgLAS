...
--- for abstract and, new, for bipolar frameworks --- the preferred construction
...
counterparts under support-extended defeat (Theorem~\ref{thm:baf}), with the
transitive-closure instantiation recovered by a splitting argument
(Corollary~\ref{cor:closure}). All proofs go through a single classical route
...
encodings on \emph{every} framework (Theorems~\ref{thm:aaf}
and~\ref{thm:baf}), and for the preferred construction provably correct via a
...
equivalence theorems for abstract and bipolar frameworks and an exact
characterization of the preferred construction, an analysis sharp enough to
...
specialization (Remark~\ref{rem:spec}), but carries no learning experiments or
equivalence theorems of its own here. $\sigma_H$ describes pooled behaviour on seven
...
learn-and-prove loop, and the same route extends the equivalence theorems to
the VAF instantiation. \emph{Dialogue}: combining the learned human policy
