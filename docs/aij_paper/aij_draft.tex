...
Table~\ref{tab:surface} gives the campaign's central result: mean recovery by
semantics, noise, and sample size on the balanced ($50\%$-positive) proportion
arm, pooled over the three completeness levels; Table~\ref{tab:pq} unpools the
completeness axis, and the imbalance arms are analysed separately in
...
($0.872\to0.619$ at $q=0.2$ as $p$ falls from $1.0$ to $0.5$). Recovery is
\emph{not} saturated by sample size alone at imperfect information --- the
$f$-axis keeps paying off precisely when labels are partial or noisy, a
...
time rises from seconds (clean) to hundreds of seconds (noisy): learning under
...
\paragraph{Generator breadth.} Re-running an anchor slice of the grid
($p\in\{1.0,0.5\}$, $q\in\{0,0.1\}$, $f\in\{20,60\}$) on a structurally
different pool: \emph{sparse} frameworks (attack count uniform in $[n,2n]$ ---
the regime of most benchmark and human-scale graphs). The clean-label sparse
cells are complete ($10$ runs each, both completeness levels): at $f{=}20$,
STB $0.994$, ADM $0.982$, CMP $0.934$, PRF $0.963$; at $f{=}60$, STB $0.999$,
ADM $1.000$, CMP $0.998$, PRF $0.975$ --- within $\pm0.03$ of the dense-pool
values at matched clean coordinates (largest deficit: CMP $-0.027$ at
$f{=}20$), so the dense generator was not propping up the headline results.
The noisy sparse cells, and two further regimes (self-attacking and larger
frameworks), are still executing on the same infrastructure
\pending{completing; the anchor-slice table will be finalized in this revision
cycle}.
...
incompleteness), imbalance immaterial, and the envelope reproduced on a sparse
generator (clean cells) and across framework families; and \emph{yes} --- on
...
\paragraph{Limitations.} The synthetic envelope is mapped on random digraphs
of moderate size; the noisy sparse cells and two further breadth regimes
(self-attacks, larger $n$) are \pending{completing}, so the generator-breadth
claim currently covers the clean sparse slice only. Grounded semantics was
