...
extension-based catalogue (committal reinstatement, cycle caution, and a
distrust of heavily attacked arguments), and a directional,
...
distrust of heavily attacked arguments --- that no semantics in the
extension-based catalogue encodes;
...
handling. Rule 4 is the psychologically distinctive hedge that no
extension-based catalogue semantics encodes (graded and ranking-based
semantics posit such attack-load sensitivity
...
What is distinctive here is the epistemic route: the hedge is \emph{induced}
from judgement data as an explicit, checkable rule --- to our knowledge for
the first time --- and it co-occurs with committal reinstatement inside a
single learned policy, rather than being posited axiomatically.
...
skeptical reading), yet on human data the learner leaves those predicates
unused; it is a residue
...
extension-learning is outside the paper's scope (Section~\ref{sec:bg-af});
...
heavily attacked arguments emerges as a learned rule rather than a posited
principle. Our further contribution is methodological (a leakage-free, gated,
...
open, now well-posed problem. VAF is covered by the unified background, its
...
would turn the post-hoc theorems of Section~\ref{sec:mech} into a
learn-and-prove loop. \emph{Dialogue}: combining the learned human policy
