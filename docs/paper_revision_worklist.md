...
**2.2 Grounded/preferred (Listings 7–8) — currently NO theorem.** The equivalence claim in
the abstract/intro implicitly covers all five semantics, but Theorem 1 covers three. We can
now state: *machine-checked on all 66,066 AFs n≤4 + 500 campaign AAFs, zero
counterexamples* (`check_heuristic_encodings.py`). Note: the arXiv's AS* definition
(subset-minimal projection) is correct — the thesis's "minimal number" wording was already
fixed; keep the subset-minimality language.
...
**five semantics** over label completeness {1.0, 0.75, 0.5} × label noise {0, 0.1, 0.2} ×
examples-per-class {10…80} × three pos/neg proportions, grouped K=5 CV, dual-surface
evaluation (matched vs complete-information test — the latter is the honest recovery
number), all leak-free and reproducible from a pinned commit. Headlines available once the
campaign completes (~days): near-perfect recovery at full information for all five
semantics; noise, amplified by partial labels, is the binding constraint; partial labels
*help* grounded at small sample sizes (cost-tie breaking).
*Caveat to state:* GRD × noise is excluded — the definite-core formulation is fragile under
polarity noise (a documented finding, not a gap).

**3.2 GRD/PRF made properly learnable at scale.** The `--learn-heuristics` route works on
the paper's small hand-crafted tasks but fails at campaign scale (our post-mortem: the flag
is costly and the eval-side domRec args were the historical failure). Two verified
alternatives worth presenting:
(a) **no-choice definite core** for grounded — ILASP learns the textbook
`in:-supported / out:-defeated` with exact recovery 1.000;
(b) **"grounded = complete + a learnable weak constraint"** — ILASP learns the minimality
preference `:~ out(X). [1@1,X]` from ordering examples in <0.1 s, oracle-equivalent on
500/500 AAFs. This is a clean, novel result connecting preference learning to semantics
selection. *Evidence:* `analysis/grd_prf_lab/`.

**3.3 ILASP 4.4.1 spurious-UNSAT bug** (no-choice GRD tasks; minimal 2-positive repro at
`analysis/grd_prf_lab/g1_definite_core/tasks/_diag_core.las`; workaround `--version=2i`).
With Mark Law co-authoring, decide: fix upstream and cite, or report as a remark.
...
- §1 "prove that the learned programs are equivalent to ASPARTIX encodings" — scope to
  {adm, cmp, stb} + point to the new BAF theorems and the exhaustive GRD/PRF checks.
...
4. **Correctness, mechanized** (Thm 1 + new BAF Thm 2 + GRD/PRF exhaustive certificates +
   the ABA bug story as a case for mechanized checking)
5. Learnability at scale (v2 campaign surfaces; GRD/PRF alternative formulations)
