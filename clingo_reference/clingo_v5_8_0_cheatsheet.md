...
Clingo supports **integer arithmetic** in terms (evaluated during grounding). Common operators:
...
- unary `-X` (negation)
- absolute value: `|X|`
- (bitwise) `&` AND, `?` OR, `^` XOR, `~X` bitwise negation

Tips:
- Use parentheses to control precedence: `(X+Y)*Z`.
- Bind computed values in the body using equality, ensuring variables are safe:
  `val(Z) :- Z = X+Y, x(X), y(Y).`
