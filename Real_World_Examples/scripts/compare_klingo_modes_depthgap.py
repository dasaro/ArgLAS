#!/usr/bin/env python3
"""Compare klingo modes/depths against learned A..G theories (strict in/1 metric).

This script is checkpoint-safe:
- writes/extends `learned_rows.csv` first (one row per example),
- appends each completed klingo evaluation to `detailed_rows.csv`,
- can be re-run on the same --run-id to resume.
"""
...
def run_cmd_capture(cmd: Sequence[str], timeout_s: float) -> Tuple[int, str, bool, float]:
    """Run command with pipe capture (more robust than PTY for long outputs)."""
    start = time.time()
    try:
        proc = subprocess.run(
            cmd,
    ...
    return rc, text, timed_out, elapsed
...
def run_klingo_models(
    ...
    # NOTE: pipe capture avoids PTY truncation artifacts seen on long runs.
    if transport == "pipe":
...
def summarize(rows: Iterable[Dict[str, str]]) -> List[Dict[str, str]]:
    ...
    for r in rows:
        key = (r["mode"], r["semantics"], r["depth"])
        a = agg.setdefault(
            key,
            {
                "n": 0.0,
        ...
        try:
            a["elapsed_s_sum"] += float(r["elapsed_s"])
        except Exception:
            pass
    out: List[Dict[str, str]] = []
    for (mode, sem, depth), a in sorted(agg.items()):
        n = max(a["n"], 1.0)
        out.append(
            {
                "mode": mode,
                ...
                "avg_elapsed_s": f"{a['elapsed_s_sum'] / n:.3f}",
            }
        )
    return out
...
def main() -> int:
    ...
    ap.add_argument("--run-id", default=f"klingo_modes_depthgap_fullin_A2G_{now_ts()}")
    ap.add_argument("--modes", nargs="+", default=["3nd", "bnm"])
    ap.add_argument("--depths", nargs="+", type=int, default=[0, 10, 25])
    ap.add_argument("--workers", type=int, default=2)
    ap.add_argument("--timeout", type=float, default=8.0)
    ap.add_argument("--clingo-timeout", type=float, default=12.0)
    ap.add_argument(
        "--transport",
        choices=("pipe",),
        default="pipe",
        help="klingo output capture transport; keep 'pipe' for robust parsing.",
    )
    ap.add_argument(
        "--base",
        default=".",
    ...
    summary_csv = run_dir / "summary_mode_sem_depth_overall.csv"
    ...
    tasks: List[Tuple[Example, str, str, int]] = []
    for ex in examples:
        for sem in SEM_FILE:
            for mode in args.modes:
                for depth in args.depths:
                    key = (ex.version, ex.bucket, ex.path, sem, mode, str(depth))
                    if key not in done:
                        tasks.append((ex, sem, mode, depth))

    print(f"[INFO] run_dir={run_dir}")
    print(f"[INFO] examples={len(examples)} tasks_total={len(examples)*len(SEM_FILE)*len(args.modes)*len(args.depths)} pending={len(tasks)}")
    print(f"[INFO] workers={args.workers} timeout={args.timeout}s")
    ...
    def worker(task: Tuple[Example, str, str, int]) -> Dict[str, str]:
        ...
        sat, sets, answers, has_unknown, timed_out, elapsed, rc = run_klingo_models(
            ...
            mode=mode,
            depth=depth,
            files=[str(sem_lp), ex.path, str(completion_lp)],
    ...
    print(f"[DONE] rows={len(rows)} summary={summary_csv}")
    return 0
