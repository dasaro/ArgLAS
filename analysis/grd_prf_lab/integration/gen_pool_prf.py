"""PRF pool generator for the integration run (winning route P1, complete-core
variant). This is a byte-faithful copy of
analysis/grd_prf_lab/p1_prf_convention/gen_pool.py with ONLY the output
directories redirected into analysis/grd_prf_lab/integration/pools/.
...
NEG  = perturbations of preferred labellings (flip_one / flip_2 / extend_in /
       ...
       under BOTH conventions that matter:
         ...
         (b) eval emptiness: [choice BG + core + completion + labelling-as-facts]
             is UNSAT.
       The pipeline pool (labelled_PRF_full) is the COMPLETE-core filtered one
       (pool_cmp in route P1; the winner). pool_adm is still generated to keep
       the RNG stream identical to the P1 reference run, then discarded.

File naming (train_test-consumable): aaf_<N>_<i>_PRF_POS_<k>.lp and
aaf_<N>_<i>_PRF_NEG_<k>.lp, each file = arg/att facts + in/out label atoms.
Incoherent AAFs (empty preferred labelling only) contribute NO files.

NEVER uses --neg_mode complete_not_target: complete-not-preferred labellings
are models of the admissible core and would make the route-P1 target theory
UNSAT.
"""
import os
import random
...
LAB = os.path.join(REPO, "analysis/grd_prf_lab/integration")
P1 = os.path.join(REPO, "analysis/grd_prf_lab/p1_prf_convention")
...
ADM_CORE = os.path.join(P1, "true_adm_core.lp")
CMP_CORE = os.path.join(P1, "true_cmp_core.lp")
...
def main():
    pools = {
        "pool_adm": (ADM_TEXT, os.path.join(LAB, "pools", "_scratch_pool_adm",
                                            "labelled_PRF_full")),
        "pool_cmp": (CMP_TEXT, os.path.join(LAB, "pools", "labelled_PRF_full")),
    }
